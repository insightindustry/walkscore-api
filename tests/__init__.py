...
.. note::
  ...
  Unit tests for **WalkScore** are written using `pytest`_ and
...
**WalkScore**, we decided to focus on practicality. That means:
  ...
  * **Coverage matters...kind of.** We have documented the primary intended
    behavior of every function in the **WalkScore** library, and the
...
Each individual test module (e.g. ``test_get_score.py``) corresponds to a
conceptual grouping of functionality. For example:

* ``test_locationscore.py`` tests the LocationScore class found in
  ``walkscore/locationscore.py``
...
**WalkScore** does not use any custom command-line options in its
test suite.
...
.. note::
  ...
  Because of the simplicity of **WalkScore**, the test suite does
...
.. note::
  ...
  The **WalkScore** test suite does support incremental testing,
